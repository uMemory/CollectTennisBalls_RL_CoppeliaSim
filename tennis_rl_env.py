...
依赖：
  pip install gymnasium numpy opencv-python coppeliasim_zmqremoteapi_client

运行前：
  - CoppeliaSim 已打开场景（不要点 Play，由代码控制启动）
  - YouBot + visionSensor 已在场景中
  - Tennis_Generate.lua 可通过 sim.callScriptFunction 调用或手动执行
"""
...
# ── 动作映射 ──
ACTION_MAP = {
    0: {'forward': 5.0, 'turn':  0.0},    # FORWARD
...
class TennisCollectorEnv(gym.Env):
    ...
    def _ball_in_active_half(self, est_bx, strict=False, est_dist=None, robot_x=None):
        ...
        strict=True  : 用于观察过滤，彻底排除对面球的干扰。
                       ...
                       margin = est_dist * UNCERTAINTY * net_proximity
        ...
        if strict and est_dist is not None and robot_x is not None:
            uncertainty = est_dist * 0.35
            net_factor = max(0.0, 1.0 - abs(robot_x) / 3.0)
            margin = uncertainty * net_factor
