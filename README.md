...
17. **巡视点 6 → 4 简化 + U 形 → C 形**：x_near 推到 5m 后中场列冗余，简化为 4 点。最早的 4 点 U 形终点在远端（巡视完后还要返程才能绕网），改为 C 形让两个端点都在近网侧（0/3），巡视终点直接对接绕网起点。新点位定义：`0=(near,Y-) → 1=(far,Y-) → 2=(far,Y+) → 3=(near,Y+)` 或反向
18. **动作切换的速度阶跃 → 转向不连贯**：env 默认 `_execute_action` 把目标速度瞬间设给电机后维持 4 sim step。RL 在 action 1（前进+左转 1.5）和 action 2（前进+右转 -1.5）频繁切换时，turn 速度瞬跳 +1.5↔-1.5 → 物理上明显抖动。在部署阶段 monkey-patch `_execute_action` 改为"4 sim step 内从上一动作速度线性渐变到目标速度"，电机输入连续不再阶跃。**仅部署生效，不影响训练**
19. **吃完球瞬间偶发盲退**：`soft_reset` 用 3 帧重复填充 frame_buffer 导致时序信息丢失，RL 偶尔输出连续 backward 动作（无目标盲退到撞网）。加兜底：连续 3 步 backward 且全程 `ball_detected=0` → 强制改为大转向探索
