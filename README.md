# 🎾 Tennis_Collector — 基于纯视觉强化学习的 YouBot 网球收集机器人

<p align="center">
  <img alt="Python" src="https://img.shields.io/badge/Python-3.11.15-3776AB?logo=python&logoColor=white">
...
> **一句话简介**：在 CoppeliaSim 真实比例（1:1）网球场内，用一台仅装备**单目摄像头**的 KUKA YouBot 移动机器人，通过 PPO 强化学习自主完成"巡场 → 发现 → 接近 → 收集"全流程，最终成功率稳定在 **85%~92%**。

## 📖 项目简介

本项目实现了一个 **完整工程化** 的纯视觉驱动强化学习捡网球系统，核心特点：

- **纯视觉感知**：YouBot 仅依赖一个前向 RGB visionSensor（1024×1024），**没有真实深度、没有全局坐标作弊**，仅依靠 HSV 颜色分割 + 轮廓像素面积估计距离与方位。
- **半场专注式 RL**：将"全场捡球"这一巨型任务拆解为"半场内单球收集"的 Gym 环境（一个 episode = 在当前半场内消除一个球），使 PPO 能在合理时间内收敛。
- **规则 + RL 混合架构**：底层 RL Agent 负责视觉闭环捡球，中层规则代码负责 S 形半场巡视，顶层规则代码负责绕网切换半场。各层职责分明、互不干扰。
...
| 模块 | 关键能力 |
|------|----------|
| 🎮 仿真环境 | CoppeliaSim 4.10.0 + ZMQ stepping 同步模式，一行 `sim.step()` 严格驱动物理与 agent |
...
├── Tennis_Spawner_Script_test.py          -- 球生成脚本测试
...
| Python | 3.11.15（开发测试版本，3.10+ 应可兼容） |
| 操作系统 | Windows / Linux |
...
2. **挂载网球生成脚本（关键步骤，否则训练中无法自动重生成网球）**：
   ...
   - 这样 Python 端才能通过 `sim.callScriptFunction('spawnBalls', ...)` 在训练运行时远程调用生成函数
   - 仓库自带 `scene.ttt` 已预挂载该脚本，直接使用可跳过本步
3. 点 **Play ▶** 启动仿真（仿真处于运行中即可，stepping 模式由 Python 端自动接管）

> 💡 **为什么必须是 Customization + Non-threaded + Lua**：
...
> ⚠️ **首次训练建议**：从头训练约 300k~500k 步，单环境 CPU 速度约 4~6 fps，预估 24~40 小时。可中途 `Ctrl+C` 安全保存并 resume。
>
...
#### 环境 V1（`tennis_rl_env.py` / `tennis_rl_envi.py`）：7 个离散动作
...
```
visionSensor RGB 图像 (1024×1024)
    ↓ 裁剪上方 40% （保留 614×1024）
    ↓ 上方裁掉是因为远处球聚集在画面中下部，上方为天空/远景噪声
HSV 转换 + 阈值分割 (H=[25,45], S=[80,255], V=[80,255])
    ↓ 形态学 OPEN + CLOSE (5×5 椭圆核) 去毛刺
findContours (RETR_EXTERNAL)
    ↓ 过滤 area < 30 像素的小斑点
按面积降序排序 → 取最大 = 最近的球
    ↓
对每个球估计世界坐标 (est_bx, est_by, est_dist):
    ball_angle_rad = atan(angle_norm × tan(FOV/2))     ← 精确反三角，非小角度近似
    est_dist = (R_ball × W) / (2 × pixel_radius × tan(FOV/2))
    est_bx = robot_x + est_dist × cos(robot_yaw + ball_angle_rad)
    est_by = robot_y + est_dist × sin(robot_yaw + ball_angle_rad)
    ↓
按活跃半场严格过滤 → 用于观测
保留全部检测结果 → 用于"对面半场球"惩罚判定
```
...
| 指标               | 含义说明                                       | 关注点                 |
|--------------------|-----------------------------------------------|------------------------|
| **fps**            | 每秒处理的仿真步数（训练速度）                  | 当前 4~6，瓶颈明显     |
