...
> **为什么是 U 形 4 点而非 S 形 6 点**：原 6 点设计含中场列 (X≈9.14m) 用于覆盖中段，但 x_near 推到 5m 后中场区已被近网列 + 底线列的 FOV 视锥充分覆盖，中场列冗余。删除后路径更短、误触发更少。损失：X∈[8,13] 且 |Y|<3 的中央死区不直接入 FOV，但球极少停留此处，且 RL 正常捡球时本身会扫过。
>
