...
- **规则 + RL 混合架构**：底层 RL Agent 负责视觉闭环捡球，中层规则代码负责 U 形 4 点半场巡视，顶层规则代码负责绕网切换半场，各层职责分明。
...
| 🤖 部署系统 | RL + 规则混合调度，绕网两段式路径 + U 形 4 点巡视 + 卡顿兜底                               |
...
U 形 4 路径点（近网列 X=±5m + 底线列 X=±15.785m × 上下两侧 Y=±7m）单程扫描，从离 YouBot 更近的端点切入。
> **S 形 6 点设计**：最初考虑到按照网球的真实比例，网球在远处时，出现在VisionSensor的像素面积可能较小而难以被检测到，故而设计了较为密集的S形路线。

> **从 S 形 6 点 改为 U 形 4 点**：主要为了避免处于另一半场靠近球网的网球对Youbot的巡视产生干扰。原 6 点设计含中场列 (X≈9.14m) 用于覆盖中段，但 x_near 推到 5m 后中场区已被近网列 + 底线列的 FOV 视锥充分覆盖，中场列冗余。删除后路径更短、误触发更少。损失：X∈[8,13] 且 |Y|<3 的中央死区不直接入 FOV，但球极少停留此处，且 RL 正常捡球时本身会扫过。
>
...
#### ② 吃完球后的回正动作（`reorient_to_safe_center`）

RL 吃完球瞬间车仍在前进姿态，下一帧若仍预测前进且当前在边界附近，会直接撞墙。每次 `info['success']=True` 后立即检查并回正：

| 条件 | 动作 |
|------|------|
| 距网 / 边界都 ≥ 1.5m | 直接返回（不在危险区） |
| 在危险区 | 原地转向使车头朝 `(active_half·4, 0)`；偏差大用纯转向，偏差小且仍贴边时低速后退 + 修正 |
| 退出 | 偏差 < 25° 且距危险 ≥ 1.0m，或 120 步上限 |
...
**关键修复**：早期 anchor=(0,0)，对贴网且跨过 X=0 的车而言，世界原点正好在网的另一侧 → 永远脱不了困。改朝己方半场内部点后此问题消除。
...
15. **吃完球瞬间 RL 仍前进姿态导致撞墙** → `info['success']=True` 后立即调 `reorient_to_safe_center(active_half)`，先把车头转向己方半场中心再交还 RL，消除惯性撞墙
16. **早期 `_escape_from_obstacle` 朝原点 (0,0) 退反而推车跨网** → 锚点改为 `(active_half·4, ry)`（己方半场内部），紧贴网时强制纯后退优先脱网
17. **巡视点 6 → 4 简化**：x_near 推到 5m 后，原 6 点 S 形中的中场列冗余（已被近网 + 底线列 FOV 视锥覆盖），简化为 U 形 4 点单程，路径短 33%，误触发概率同步降低
18. **网保护误阻挡 RL 捡贴网球**：固定 1.2m 安全半径让 RL 永远无法接近贴网（己方半场内）的球。引入"眼前有近距离球"信号（`ball_detected + ball_size>0.20 + |angle|<0.4`），触发后把网/边界保护半径同时缩到 0.4m，让 RL 合法接近，同时由 ELIM_DIST=0.4m 物理上保证不越网
