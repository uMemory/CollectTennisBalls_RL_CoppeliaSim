"""
tennis_collector_v3.py
======================
阶段测试：全局坐标导航 + 全局距离消除 + 球网绕行

v3 改动摘要（相对 v2）：
  1. get_youbot_pose() 改用变换矩阵提取 yaw，彻底消除万向锁问题
     车头方向 = 局部 -Z 轴 → 世界 XY 投影 → atan2
  2. 废弃 navigate_to_waypoint（两阶段硬切控制，易震荡）
     统一用 navigate_to_target()：低通滤波 + drive_to 混合驱动
  3. 简化绕网路径：3 路径点，bypass_y 降至 6.8m
  4. 球网附近球（|bx| < 0.5m）特殊处理

球网约束（来自 lua 场景）：
  - Net_Collision_Wall: X=0, Y ∈ [-6.40, +6.40]m, 高 0.914m, 厚 0.08m
  - 网柱 Y = ±6.40m，网柱外侧（|Y| > 6.40m）无障碍可穿越 X=0
...
# --- 导航参数 ---
BASE_SPEED    = 7.5   # rad/s 基础轮速
TURN_SPEED    = 3.5   # rad/s 最大转向轮速
...
# --- 球网绕行参数（来自 lua 场景几何） ---
NET_WALL_Y    = 6.40    # m，碰撞墙 Y 方向半宽（netW/2）
NET_BYPASS_Y  = 7.40    # m，绕行通道 Y 坐标（网柱外侧，留 0.4m 余量）
COURT_X_HALF  = 11.885  # m，球场半长（CL/2）
NET_NEAR_THRESHOLD = 0.5  # m，球距球网 X<此值视为"贴网球"
WAYPOINT_REACHED = 0.50   # m，到达路径点的距离容差
...
def get_youbot_pose():
    ...
    使用 4x4 变换矩阵提取车头方向，完全避免万向锁问题。

    实测确认的坐标映射（初始姿态 Rx=-90°, Ry=0°, Rz=-90°）：
      局部 X → 世界 Z（朝上）
      局部 Y → 世界 X
      局部 Z → 世界 Y
      车头前进方向 = 局部 -Z = 世界 -Y（初始时）

    getObjectMatrix 返回 12 元素行优先矩阵 [R|t]：
      m = [r00, r01, r02, tx,
           r10, r11, r12, ty,
           r20, r21, r22, tz]
      局部 Z 轴在世界 = (m[2], m[6], m[10])
      车头 = 局部 -Z   = (-m[2], -m[6], -m[10])

    yaw = atan2(-m[6], -m[2])  即车头方向在世界 XY 平面的角度
    """
    if youbot_h is None:
        return 0.0, 0.0, 0.0
...
def navigate_to_target(tx, ty, reach_dist, label="TARGET", max_iter=2000):
    """
    统一导航函数：低通滤波 + drive_to 混合驱动。
    用于路径点导航和网球接近，区别仅在 reach_dist。

    参数：
        tx, ty     : 目标世界坐标
        reach_dist : 到达判定距离（路径点用 WAYPOINT_REACHED，网球用 ELIM_GLOBAL_DIST）
        label      : 日志标签
        max_iter   : 最大迭代步数

    返回：True=到达  |  False=超时/放弃
    """
    LOOP_DT  = 0.05
    ALPHA    = 0.15   # 低通滤波系数
...
def plan_bypass_waypoints(rx, ry, bx, by):
    ...
    球网碰撞墙：X=0，|Y| ≤ 6.40m
    绕行通道：  |Y| > 6.40m（网柱外侧无障碍）

    路径：
      WP1: (rx_safe,  bypass_y)  ← 纵向移到侧道，X 保持在当前侧
      WP2: (bx_safe,  bypass_y)  ← 横向穿越 X=0 到目标半场
      WP3: (bx_safe,  by)        ← 纵向移到球的 Y 附近

    bypass_y 选择：取与当前 ry 同号的侧道（最短路径）；
    若 |ry| 已经超过 bypass_y，直接用 ry 的符号。
    """
    # 选最近侧道
    ...
    waypoints = [
        (rx_safe, bypass_y),      # WP1: 纵向移到侧道
        (bx_safe, bypass_y),      # WP2: 横向穿越到目标半场
        (bx_safe, by_clamped),    # WP3: 纵向移到球附近
    ]
...
def navigate_to_ball(ball_h, ball_name):
    ...
    if needs_net_bypass(rx, bx):
        ...
        print(f"  ⚠️ 球贴近球网（bx={bx:.2f}），从侧面接近...")
    ...
    for step in range(MAX_ITER):
        ...
        if stall_count >= STALL_LIMIT:
            print(f"  ⚠️ step={step} 卡顿！dist={dist:.2f}m 未改善，后退恢复...")
            set_motors(-RECOVER_SPEED, -RECOVER_SPEED,
                       -RECOVER_SPEED, -RECOVER_SPEED)
            time.sleep(1.0)
            stop_robot()
